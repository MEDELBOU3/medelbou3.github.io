...
loadMostViewedSection: async (sectionDiv, sectionConfig) => {
    ...
    try {
        ...
        if (isHorizontal) {
            // --- MODIFIED: Pass true for the new addRankings parameter ---
            ...
            // You might need a similar modification for renderTmdbCards if you want ranks there too
    ...
    renderHorizontalCards: (items, containerElement, defaultType = null, showTrailerButton = false, addRankings = false) => { // <<< ADD addRankings parameter
        ...
        if (!items || items.length === 0) {
            ...
            // Update scroll buttons even if empty for horizontal sections
            const parentWrapper = containerElement.closest('.horizontal-scroll-wrapper');
            if(parentWrapper) {
               const prevBtn = parentWrapper.querySelector('.h-scroll-btn.prev');
               const nextBtn = parentWrapper.querySelector('.h-scroll-btn.next');
               if(prevBtn && nextBtn) App.updateHScrollButtons(containerElement, prevBtn, nextBtn);
            }
            return;
        }

        items.forEach((item, index) => { // <<< Get index here
            ...
            const trailerButtonHtml = showTrailerButton
                ? `<button class="h-card-play-trailer-btn" aria-label="Play Trailer for ${title}" data-item-id="${item.id}" data-item-type="${itemType}"><i class="bi bi-play-fill"></i></button>`
                : '';

            // Watchlist Button
            const isInList = Watchlist.isInWatchlist(item.id, itemType);
            const watchlistBtnHtml = `
                <button class="btn action-btn watchlist-btn ${isInList ? 'in-watchlist' : ''}" title="${isInList ? 'In Watchlist (Click to remove)' : 'Add to Watchlist'}" aria-label="${isInList ? 'Remove from Watchlist' : 'Add to Watchlist'}" data-item-id="${item.id}" data-item-type="${itemType}" data-item-title="${Utils.escapeHtml(item.title || item.name || '')}" data-item-poster="${item.poster_path || null}" data-item-backdrop="${item.backdrop_path || null}" data-item-rating="${item.vote_average || null}" style="top: 0.6rem; right: 0.6rem;">
                    <i class="bi ${isInList ? 'bi-bookmark-check-fill' : 'bi-bookmark-plus'}"></i>
                </button>`;

            // --- NEW: Generate Ranking HTML ---
            let rankHtml = '';
            if (addRankings) {
                const rankNumber = index + 1;
                // Add a specific class for #1 for potential special styling
                const rankClass = rankNumber === 1 ? 'rank-1' : '';
                rankHtml = `<div class="ranking-badge ${rankClass}">#${rankNumber}</div>`;
            }
            // --- END NEW ---

            // --- MODIFIED: Inject rankHtml ---
            // Make card relative for positioning badge
            cardLink.style.position = 'relative';
            cardLink.innerHTML = rankHtml + imageHtml + overlayHtml + trailerButtonHtml + watchlistBtnHtml;
            // --- END MODIFIED ---
